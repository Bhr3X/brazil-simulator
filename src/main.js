...
class GameApp {
  ...
  initUI() {
    ...
    if (timeBtn) {
      timeBtn.addEventListener('click', () => this.cycleTimeOfDay());
    }
  ...
  cycleTimeOfDay() {
    ...
    // Harmonize sim clock with manual time-of-day toggle (forward-only during active run)
    ...
    if (this.game && this.game.clock) {
      ...
      this.game.clock.setHour(targetH, !!this.game.isRunActive);
    }
