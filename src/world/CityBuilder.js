...
export class CityBuilder {
  ...
  build() {
    ...
    this.buildPaulaFerreiraCorridorCommercesAndSobrados();
    this.buildSeteBarrasSector();
    this.buildEdgarFaccoPetronioPortelaExtension();
    this.buildEdgarFaccoExtendedCommerces();
    ...
    this.buildPetronioPortelaFurtherSouthExtension();
  ...
  buildWalkablePastelaria(x, y, z) {
    ...
    const wallMat = new THREE.MeshLambertMaterial({
      map: this.textures.createPaintedWall('#fef08a', 2, 2)
    });
    const intTileMat = new THREE.MeshLambertMaterial({ color: 0xffffff });
    const floorMat = new THREE.MeshLambertMaterial({
      map: this.textures.createPisoPastelaria(4, 4)
    });
    const ceilingMat = new THREE.MeshLambertMaterial({ color: 0x827d75 });
    const chromeMat = new THREE.MeshLambertMaterial({ color: 0xd4d4d8 });
    const counterFormicaMat = new THREE.MeshLambertMaterial({ color: 0x9a3412 }); // Amber formica
    const counterTopMat = new THREE.MeshLambertMaterial({ color: 0xf1f5f9 }); // Stainless steel top
    const pastelMat = new THREE.MeshLambertMaterial({ color: 0xeab308 }); // Golden fried pastry
    const caneMat = new THREE.MeshLambertMaterial({ color: 0x65a30d }); // Sugarcane stalks
    const chairRedMat = new THREE.MeshLambertMaterial({ color: 0xdc2626 });
    const chairYellowMat = new THREE.MeshLambertMaterial({ color: 0xfacc15 });
    ...
    // 2. Ceiling / Laje with water tank
    const ceiling = new THREE.Mesh(new THREE.BoxGeometry(w, 0.25, d), ceilingMat);
    ceiling.position.set(x, roofY, z);
    this.scene.add(ceiling);
    this.buildWaterTank(x - 2.0, roofY + 0.1, z + 1.5);

    // 3. Back Wall (Z = z + halfD) with Menu Board
    const backWall = new THREE.Mesh(new THREE.BoxGeometry(w, h, 0.3), intTileMat);
    backWall.position.set(x, floorY + h / 2, z + halfD);
    this.scene.add(backWall);
    this.physics.addBoxCollider(
      new THREE.Vector3(x - halfW, floorY, z + halfD - 0.2),
    ...
    // Large Menu Blackboard on Back Wall
    const menuMat = new THREE.MeshBasicMaterial({ map: this.textures.createPastelariaMenuBoard() });
    const menuBoard = new THREE.Mesh(new THREE.PlaneGeometry(3.6, 1.8), menuMat);
    menuBoard.position.set(x + 1.2, floorY + 2.6, z + halfD - 0.16);
    menuBoard.rotation.y = Math.PI;
    this.scene.add(menuBoard);

    // Kitchen pass-through door / curtain on back wall
    const kitchenDoor = new THREE.Mesh(
      new THREE.BoxGeometry(1.4, 2.6, 0.05),
      new THREE.MeshLambertMaterial({ color: 0x334155 })
    );
    kitchenDoor.position.set(x - 3.2, floorY + 1.3, z + halfD - 0.14);
    this.scene.add(kitchenDoor);

    // 4. Left Wall (West: X = x - halfW, adjacent to Lotérica)
    const leftWall = new THREE.Mesh(new THREE.BoxGeometry(0.3, h, d), wallMat);
    leftWall.position.set(x - halfW, floorY + h / 2, z);
    this.scene.add(leftWall);
    this.physics.addBoxCollider(
    ...
    // 6. Front Facade with Wide Open Central Entrance (doorW = 6.8m)
    const doorW = 6.8;
    const sideWallW = (w - doorW) / 2; // 2.1m each side
    ...
    this.physics.addBoxCollider(
      new THREE.Vector3(x - halfW, floorY, z - halfD - 0.15),
      new THREE.Vector3(x - halfW + sideWallW, roofY, z - halfD + 0.15),
      'solid'
    );
    ...
    this.scene.add(fRight);
    this.physics.addBoxCollider(
      new THREE.Vector3(x + halfW - sideWallW, floorY, z - halfD - 0.15),
      new THREE.Vector3(x + halfW, roofY, z - halfD + 0.15),
      'solid'
    );

    // Overhead lintel
    const lintel = new THREE.Mesh(new THREE.BoxGeometry(doorW, 1.4, 0.3), wallMat);
    lintel.position.set(x, floorY + h - 0.7, z - halfD);
    this.scene.add(lintel);

    // Rolled-up steel shutter door overhead
    const shutterMat = new THREE.MeshLambertMaterial({ map: this.textures.createPortaAco('#475569') });
    const shutter = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.22, doorW - 0.1, 12), shutterMat);
    shutter.rotation.z = Math.PI / 2;
    shutter.position.set(x, floorY + h - 1.45, z - halfD);
    this.scene.add(shutter);

    // Signboard "PASTELARIA DO BETO"
    const pastSignMat = new THREE.MeshBasicMaterial({ map: this.textures.createPastelariaBetoSign() });
    const pastSign = new THREE.Mesh(new THREE.BoxGeometry(5.8, 1.25, 0.1), pastSignMat);
    pastSign.position.set(x, floorY + h - 0.75, z - halfD - 0.18);
    this.scene.add(pastSign);

    // Red & yellow awning over entrance
    const pastAwningMat = new THREE.MeshLambertMaterial({ color: 0xdc2626 });
    const pastAwning = new THREE.Mesh(new THREE.BoxGeometry(7.2, 0.1, 1.6), pastAwningMat);
    pastAwning.rotation.x = -0.25;
    pastAwning.position.set(x, floorY + 3.2, z - halfD - 0.8);
    this.scene.add(pastAwning);

    // 7. Interior Warm Lighting (Pendant lights & Estufa glow)
    const intLight = new THREE.PointLight(0xffe6aa, 2.4, 16, 1.1);
    intLight.position.set(x + 1.0, floorY + 3.6, z);
    this.scene.add(intLight);

    const intLight2 = new THREE.PointLight(0xfff7ed, 1.8, 12, 1.1);
    intLight2.position.set(x - 2.0, floorY + 3.6, z + 2.0);
    this.scene.add(intLight2);

    // Pendant shades
    [x + 1.0, x - 2.0].forEach(lx => {
      const shade = new THREE.Mesh(
        new THREE.ConeGeometry(0.35, 0.22, 8, 1, true),
        new THREE.MeshLambertMaterial({ color: 0x991b1b, side: THREE.DoubleSide })
      );
      shade.position.set(lx, floorY + 3.7, z + 1.0);
      this.scene.add(shade);
    });

    // 8. Service Counter (Balcão em L)
    const counterW = 0.8;
    const counterH = 1.05;
    const counterD = 5.0;
    const counterBase = new THREE.Mesh(new THREE.BoxGeometry(counterW, counterH, counterD), counterFormicaMat);
    counterBase.position.set(x - 1.5, floorY + counterH / 2, z + 0.5);
    this.scene.add(counterBase);

    const counterTop = new THREE.Mesh(new THREE.BoxGeometry(counterW + 0.15, 0.08, counterD + 0.2), counterTopMat);
    counterTop.position.set(x - 1.5, floorY + counterH + 0.04, z + 0.5);
    this.scene.add(counterTop);

    // Counter solid collider
    this.physics.addBoxCollider(
      new THREE.Vector3(x - 1.5 - counterW / 2, floorY, z + 0.5 - counterD / 2),
      new THREE.Vector3(x - 1.5 + counterW / 2, floorY + counterH, z + 0.5 + counterD / 2),
      'solid'
    );

    // Return counter at the back (Z = z + 3.0)
    const retW = 3.5;
    const retBase = new THREE.Mesh(new THREE.BoxGeometry(retW, counterH, 0.8), counterFormicaMat);
    retBase.position.set(x - 3.25, floorY + counterH / 2, z + 2.6);
    this.scene.add(retBase);
    const retTop = new THREE.Mesh(new THREE.BoxGeometry(retW + 0.1, 0.08, 0.9), counterTopMat);
    retTop.position.set(x - 3.25, floorY + counterH + 0.04, z + 2.6);
    this.scene.add(retTop);
    this.physics.addBoxCollider(
      new THREE.Vector3(x - 3.25 - retW / 2, floorY, z + 2.6 - 0.4),
      new THREE.Vector3(x - 3.25 + retW / 2, floorY + counterH, z + 2.6 + 0.4),
      'solid'
    );

    // 9. Estufa de Pastéis Aquecida (Curved Glass Display Warmer)
    const estufaGeo = new THREE.BoxGeometry(0.7, 0.55, 1.4);
    const estufaGlass = new THREE.Mesh(
      estufaGeo,
      new THREE.MeshLambertMaterial({ color: 0xffedd5, transparent: true, opacity: 0.65 })
    );
    estufaGlass.position.set(x - 1.5, floorY + counterH + 0.32, z - 0.5);
    this.scene.add(estufaGlass);

    // Warm golden glow inside the estufa
    const estufaGlow = new THREE.PointLight(0xf59e0b, 2.0, 4.0);
    estufaGlow.position.set(x - 1.5, floorY + counterH + 0.35, z - 0.5);
    this.scene.add(estufaGlow);

    // Wire rack inside estufa
    const rack = new THREE.Mesh(new THREE.BoxGeometry(0.55, 0.02, 1.2), chromeMat);
    rack.position.set(x - 1.5, floorY + counterH + 0.18, z - 0.5);
    this.scene.add(rack);

    // Golden pastéis displayed inside the estufa
    for (let p = 0; p < 4; p++) {
      const past = new THREE.Mesh(new THREE.BoxGeometry(0.26, 0.04, 0.16), pastelMat);
      past.rotation.y = 0.2 * (p % 2 === 0 ? 1 : -1);
      past.position.set(x - 1.5, floorY + counterH + 0.22, z - 0.95 + p * 0.3);
      this.scene.add(past);
    }

    // 10. Moenda Elétrica de Caldo de Cana & Sugarcane Props
    const moendaBase = new THREE.Mesh(
      new THREE.BoxGeometry(0.55, 0.55, 0.7),
      new THREE.MeshLambertMaterial({ color: 0x15803d }) // Green chassis
    );
    moendaBase.position.set(x - 1.5, floorY + counterH + 0.3, z + 1.5);
    this.scene.add(moendaBase);

    // Stainless steel rollers and feed chute
    const roller = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 0.3, 10), chromeMat);
    roller.rotation.x = Math.PI / 2;
    roller.position.set(x - 1.35, floorY + counterH + 0.38, z + 1.5);
    this.scene.add(roller);

    const chute = new THREE.Mesh(new THREE.BoxGeometry(0.25, 0.04, 0.3), chromeMat);
    chute.rotation.z = -0.3;
    chute.position.set(x - 1.25, floorY + counterH + 0.42, z + 1.5);
    this.scene.add(chute);

    // Sugarcane stalks basket beside the press
    for (let c = 0; c < 6; c++) {
      const cane = new THREE.Mesh(new THREE.CylinderGeometry(0.035, 0.035, 1.6, 8), caneMat);
      cane.rotation.z = 0.1 + (c % 2) * 0.08;
      cane.position.set(x - 2.1 + (c % 3) * 0.09, floorY + 0.8, z + 2.1 + Math.floor(c / 3) * 0.09);
      this.scene.add(cane);
    }

    // Pitcher of Iced Caldo de Cana
    const jug = new THREE.Mesh(
      new THREE.CylinderGeometry(0.12, 0.1, 0.35, 8),
      new THREE.MeshLambertMaterial({ color: 0x84cc16, transparent: true, opacity: 0.8 })
    );
    jug.position.set(x - 1.5, floorY + counterH + 0.22, z + 2.1);
    this.scene.add(jug);

    // 11. Condiments on Counter (Vinagrete, Pimenta, Guardanapos)
    const potMat = new THREE.MeshLambertMaterial({ color: 0xffffff, transparent: true, opacity: 0.7 });
    const vinagretePot = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 0.12, 8), potMat);
    vinagretePot.position.set(x - 1.5, floorY + counterH + 0.1, z + 0.5);
    this.scene.add(vinagretePot);

    const spoon = new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.16, 0.02), chromeMat);
    spoon.rotation.z = 0.2;
    spoon.position.set(x - 1.5, floorY + counterH + 0.14, z + 0.5);
    ...
    mustard.position.set(x - 1.5, floorY + counterH + 0.13, z + 0.7);
    this.scene.add(mustard);

    const ketchup = new THREE.Mesh(
      new THREE.CylinderGeometry(0.035, 0.035, 0.18, 8),
      new THREE.MeshLambertMaterial({ color: 0xdc2626 })
    );
    ketchup.position.set(x - 1.5, floorY + counterH + 0.13, z + 0.8);
    this.scene.add(ketchup);

    // Napkin dispenser
    const napkinBox = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.16, 0.12), chromeMat);
    napkinBox.position.set(x - 1.5, floorY + counterH + 0.12, z + 0.3);
    this.scene.add(napkinBox);

    // 12. Counter Barstools
    [-1.2, 0.8].forEach(sz => {
      const stoolBase = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.2, 0.04, 10), chromeMat);
      stoolBase.position.set(x - 0.75, floorY + 0.02, z + sz);
      this.scene.add(stoolBase);
      const stoolPole = new THREE.Mesh(new THREE.CylinderGeometry(0.035, 0.035, 0.68, 8), chromeMat);
      stoolPole.position.set(x - 0.75, floorY + 0.35, z + sz);
      this.scene.add(stoolPole);
      const stoolSeat = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.2, 0.06, 10), chairRedMat);
      stoolSeat.position.set(x - 0.75, floorY + 0.72, z + sz);
      this.scene.add(stoolSeat);
    });

    // 13. Salão Dining Area: 2 Dining Tables with Chairs & Props
    const createDiningTable = (tx, tz, chairMat, pastelCount) => {
      // Pedestal & Tabletop
      const tablePost = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 0.7, 8), chromeMat);
      tablePost.position.set(tx, floorY + 0.35, tz);
      this.scene.add(tablePost);
      const tableBase = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 0.3, 0.04, 12), chromeMat);
      tableBase.position.set(tx, floorY + 0.02, tz);
      this.scene.add(tableBase);
      const tableTop = new THREE.Mesh(new THREE.BoxGeometry(1.0, 0.06, 1.0), tableMat);
      tableTop.position.set(tx, floorY + 0.73, tz);
      this.scene.add(tableTop);

      // Table collider
      this.physics.addBoxCollider(
        new THREE.Vector3(tx - 0.5, floorY, tz - 0.5),
        new THREE.Vector3(tx + 0.5, floorY + 0.8, tz + 0.5),
        'solid'
      );

      // 2 Chairs (North & South)
      [-0.75, 0.75].forEach(cz => {
        const chairSeat = new THREE.Mesh(new THREE.BoxGeometry(0.42, 0.05, 0.42), chairMat);
        chairSeat.position.set(tx, floorY + 0.45, tz + cz);
        this.scene.add(chairSeat);

        const chairBack = new THREE.Mesh(new THREE.BoxGeometry(0.42, 0.42, 0.04), chairMat);
        chairBack.position.set(tx, floorY + 0.72, tz + cz + (cz > 0 ? 0.2 : -0.2));
        this.scene.add(chairBack);

        // Legs
        [-0.18, 0.18].forEach(lx => {
          const leg = new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.02, 0.45, 6), chromeMat);
          leg.position.set(tx + lx, floorY + 0.225, tz + cz);
          this.scene.add(leg);
        });
      });

      // Tabletop Food Props: Plate with 30cm pastel & Caldo de cana
      const plate = new THREE.Mesh(new THREE.CylinderGeometry(0.24, 0.22, 0.03, 12), tableMat);
      plate.position.set(tx - 0.1, floorY + 0.77, tz);
      this.scene.add(plate);

      for (let i = 0; i < pastelCount; i++) {
        const past = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.04, 0.16), pastelMat);
        past.position.set(tx - 0.1, floorY + 0.8 + i * 0.04, tz);
        this.scene.add(past);
      }

      // Cup of Caldo de Cana with straw
      const cup = new THREE.Mesh(
        new THREE.CylinderGeometry(0.045, 0.035, 0.16, 8),
        new THREE.MeshLambertMaterial({ color: 0x84cc16, transparent: true, opacity: 0.85 })
      );
      cup.position.set(tx + 0.25, floorY + 0.84, tz - 0.18);
      this.scene.add(cup);

      const straw = new THREE.Mesh(
        new THREE.CylinderGeometry(0.008, 0.008, 0.24, 6),
        new THREE.MeshLambertMaterial({ color: 0xdc2626 })
      );
      straw.rotation.z = 0.25;
      straw.position.set(tx + 0.28, floorY + 0.94, tz - 0.18);
      this.scene.add(straw);

      // Mini napkin holder
      const nap = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.12, 0.08), chromeMat);
      nap.position.set(tx + 0.22, floorY + 0.82, tz + 0.2);
      this.scene.add(nap);
    };

    // Table 1 (North dining spot) with red chairs
    createDiningTable(x + 2.4, z - 2.2, chairRedMat, 1);

    // Table 2 (South dining spot) with yellow chairs
    createDiningTable(x + 2.4, z + 2.0, chairYellowMat, 2);
  }
  ...
  buildMidClassHouse(x, y, z) {
    ...
    const h = 5.2;
    const d = 6.0;
    // Solid elevated foundation: floor sits at Y = 0.28, completely above street/sidewalk level (0.25)
    const floorY = 0.28;
    const posY = floorY + h / 2;
    ...
    const lintel = new THREE.Mesh(new THREE.BoxGeometry(doorW, h - doorH, 0.3), wallMat);
    lintel.position.set(x, floorY + doorH + (h - doorH) / 2, z - d / 2 + 0.15);
    this.scene.add(lintel);

    // Solid foundation slab: spans Y from -0.06 to 0.28 (0.34m thick solid concrete)
    // Encapsulates and occludes any overlapping sidewalk or subfloor planes, completely eliminating z-fighting
    const slabGeo = new THREE.BoxGeometry(w, 0.34, d);
    const slabMat = new THREE.MeshLambertMaterial({ color: 0x3a3d40 });
    const slab = new THREE.Mesh(slabGeo, slabMat);
    slab.position.set(x, 0.11, z);
    this.scene.add(slab);

    // Interior floor (Parquet / Taco de madeira) mounted firmly on top of the slab
    const floorMesh = new THREE.Mesh(new THREE.PlaneGeometry(w - 0.4, d - 0.4), woodFloorMat);
    floorMesh.rotation.x = -Math.PI / 2;
    floorMesh.position.set(x, floorY + 0.002, z);
    this.scene.add(floorMesh);

    // Dedicated walkable floor collider across entire house interior
    this.physics.addBoxCollider(
      new THREE.Vector3(x - w / 2, 0, z - d / 2),
      new THREE.Vector3(x + w / 2, floorY + 0.05, z + d / 2),
      'walkable'
    );

    // Granite entrance threshold / soleira at front doorway
    const soleiraMat = new THREE.MeshLambertMaterial({ color: 0x2b2b2b });
    const soleira = new THREE.Mesh(new THREE.BoxGeometry(doorW + 0.2, 0.06, 0.35), soleiraMat);
    soleira.position.set(x, 0.26, z - d / 2 + 0.05);
    this.scene.add(soleira);

    // Ceiling / Flat Roof
    const roof = new THREE.Mesh(new THREE.BoxGeometry(w + 0.4, 0.3, d + 0.4), ceilingMat);
    roof.position.set(x, floorY + h + 0.15, z);
    this.scene.add(roof);

    // Blue Fortlev water tank on roof
    this.buildWaterTank(x + 1.2, floorY + h + 0.3, z + 0.5);
    ...
    table.position.set(x - 2.0, floorY + 0.2, z - 0.1);
    ...
    boleto.position.set(x - 2.2, floorY + 0.41, z - 0.1);
    ...
    tvRack.position.set(x + 2.0, floorY + 0.3, z - 0.1);
    this.scene.add(tvRack);

    // CRT TV Body
    const tvMat = new THREE.MeshLambertMaterial({ color: 0x222222 });
    const tv = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.65, 0.55), tvMat);
    tv.position.set(x + 2.0, floorY + 0.925, z - 0.1);
    this.scene.add(tv);
    ...
    tvScreen.position.set(x + 2.0, floorY + 0.925, z - 0.38);
    ...
    crochet.rotation.x = -Math.PI / 2;
    crochet.position.set(x + 2.0, floorY + 1.26, z - 0.1);
    this.scene.add(crochet);
    ...
    warmLight.position.set(x, floorY + 2.8, z);
    this.scene.add(warmLight);

    // 6. Solid Physics Colliders for Walls & Furnishings
    // Back wall
    this.physics.addBoxCollider(
      new THREE.Vector3(x - w / 2, floorY, z + d / 2 - 0.4),
      new THREE.Vector3(x + w / 2, floorY + h, z + d / 2 + 0.1),
      'solid'
    );
    // Left wall
    this.physics.addBoxCollider(
      new THREE.Vector3(x - w / 2 - 0.1, floorY, z - d / 2),
      new THREE.Vector3(x - w / 2 + 0.4, floorY + h, z + d / 2),
      'solid'
    );
    // Right wall
    this.physics.addBoxCollider(
      new THREE.Vector3(x + w / 2 - 0.4, floorY, z - d / 2),
      new THREE.Vector3(x + w / 2 + 0.1, floorY + h, z + d / 2),
      'solid'
    );
    // Front wall Left
    this.physics.addBoxCollider(
      new THREE.Vector3(x - w / 2, floorY, z - d / 2 - 0.1),
      new THREE.Vector3(x - doorW / 2, floorY + h, z - d / 2 + 0.4),
      'solid'
    );
    // Front wall Right
    this.physics.addBoxCollider(
      new THREE.Vector3(x + doorW / 2, floorY, z - d / 2 - 0.1),
      new THREE.Vector3(x + w / 2, floorY + h, z - d / 2 + 0.4),
      'solid'
    );
    // Sofa & TV colliders
    this.physics.addBoxCollider(
      new THREE.Vector3(x - 3.2, floorY, z + 0.7),
      new THREE.Vector3(x - 0.8, floorY + 1.2, z + 1.7),
      'solid'
    );
    this.physics.addBoxCollider(
      new THREE.Vector3(x + 1.2, floorY, z - 0.4),
      new THREE.Vector3(x + 2.8, floorY + 1.5, z + 0.2),
      'solid'
    );

    // 7. Interactive Front Wooden Door (Opens when punched with [E] or hit)
    const doorPivot = new THREE.Group();
    doorPivot.position.set(x - doorW / 2, floorY, z - d / 2 + 0.15);
    ...
    const doorCollider = this.physics.addBoxCollider(
      new THREE.Vector3(x - doorW / 2, floorY, z - d / 2 - 0.2),
      new THREE.Vector3(x + doorW / 2, floorY + doorH, z - d / 2 + 0.3),
      'solid'
    );
    ...
    this.interactiveDoors.push({
      ...
      position: new THREE.Vector3(x, floorY + 1.2, z - d / 2 - 0.4),
      maxDist: 2.4,
      doorMesh: doorPivot,
      collider: doorCollider,
      isOpen: false,
  ...
  buildExpandedStreetsAndDetails() {
    ...
    this.buildMidClassHouse(-8.0, 0.25, -12.0);
  ...
  buildPaulaFerreiraFreguesiaExtension() {
    ...
    // Concrete Retaining Walls (Muros de Arrimo) behind stepped buildings
    const wallMat = new THREE.MeshLambertMaterial({
      map: this.textures.createReboco('#7d776f', 8, 4)
    });

    // West retaining wall behind buildings
    const wallW = new THREE.Mesh(new THREE.BoxGeometry(0.5, 6.0, slopeHypot), wallMat);
    wallW.rotation.x = -slopeAngle;
    wallW.position.set(-14.0, 3.8, 115.0);
    this.scene.add(wallW);

    // East retaining wall behind buildings
    const wallE = new THREE.Mesh(new THREE.BoxGeometry(0.5, 6.0, slopeHypot), wallMat);
    wallE.rotation.x = -slopeAngle;
    wallE.position.set(17.0, 3.8, 115.0);
    this.scene.add(wallE);

    // Sidewalk stone curbs framing the street
    const curbMat = new THREE.MeshLambertMaterial({ color: 0x94a3b8 });
    const curbW = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.25, slopeHypot), curbMat);
    curbW.rotation.x = -slopeAngle;
    curbW.position.set(-3.5, 4.35, 115.0);
    this.scene.add(curbW);

    const curbE = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.25, slopeHypot), curbMat);
    curbE.rotation.x = -slopeAngle;
    curbE.position.set(6.5, 4.35, 115.0);
  ...
  // 19.3 Continuous Street View Commerces & Terraced Sobrados along Rua Paula Ferreira (Z = 75 to 155)
  buildPaulaFerreiraCorridorCommercesAndSobrados() {
    const calcElevation = (z) => ((z - 75.0) / 80.0) * 8.5;
    const chromeMat = new THREE.MeshLambertMaterial({ color: 0xd4d4d8 });
    const roofMat = new THREE.MeshLambertMaterial({ map: this.textures.createTelhaOndulada(4, 4) });

    // =========================================================================
    // WEST SIDE OF PAULA FERREIRA (X: -13.5 to -6.0)
    // =========================================================================

    // 1. Barbearia do Seu Antônio (Z = 78.0 to 92.0, Center Z = 85.0)
    const barbZ = 85.0;
    const barbY = calcElevation(barbZ);
    const barbGeo = new THREE.BoxGeometry(7.5, 5.2, 14.0);
    const barbMat = new THREE.MeshLambertMaterial({
      map: this.textures.createPaintedWall('#1e293b', 2, 2)
    });
    const barbMesh = new THREE.Mesh(barbGeo, barbMat);
    barbMesh.position.set(-9.75, barbY + 2.6, barbZ);
    this.scene.add(barbMesh);

    // Barbearia Sign
    const barbSignMat = new THREE.MeshBasicMaterial({ map: this.textures.createBarbeariaSign() });
    const barbSign = new THREE.Mesh(new THREE.BoxGeometry(0.1, 1.2, 6.0), barbSignMat);
    barbSign.position.set(-5.95, barbY + 3.8, barbZ);
    this.scene.add(barbSign);

    // 3D Barber Pole (Helical striped cylinder)
    const poleMat = new THREE.MeshBasicMaterial({ map: this.textures.createBarberPoleTexture() });
    const barberPole = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.18, 1.3, 16), poleMat);
    barberPole.position.set(-5.85, barbY + 2.4, barbZ - 3.8);
    this.scene.add(barberPole);

    // Barber Pole brackets and chrome cap
    const capGeo = new THREE.SphereGeometry(0.22, 12, 12);
    const poleCap = new THREE.Mesh(capGeo, chromeMat);
    poleCap.position.set(-5.85, barbY + 3.1, barbZ - 3.8);
    this.scene.add(poleCap);

    const bracketGeo = new THREE.BoxGeometry(0.25, 0.08, 0.15);
    const brkTop = new THREE.Mesh(bracketGeo, chromeMat);
    brkTop.position.set(-5.95, barbY + 2.9, barbZ - 3.8);
    this.scene.add(brkTop);
    const brkBot = new THREE.Mesh(bracketGeo, chromeMat);
    brkBot.position.set(-5.95, barbY + 1.8, barbZ - 3.8);
    this.scene.add(brkBot);

    // Barber interior chair
    const chairBase = new THREE.Mesh(new THREE.CylinderGeometry(0.4, 0.45, 0.15, 12), chromeMat);
    chairBase.position.set(-8.5, barbY + 0.15, barbZ);
    this.scene.add(chairBase);
    const chairSeat = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.2, 0.7), new THREE.MeshLambertMaterial({ color: 0x991b1b }));
    chairSeat.position.set(-8.5, barbY + 0.65, barbZ);
    this.scene.add(chairSeat);
    const chairBack = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.7, 0.15), new THREE.MeshLambertMaterial({ color: 0x991b1b }));
    chairBack.position.set(-8.5, barbY + 1.05, barbZ + 0.3);
    this.scene.add(chairBack);

    // Warm Interior Point Light
    const barbLight = new THREE.PointLight(0xfff1d6, 1.2, 8);
    barbLight.position.set(-8.0, barbY + 2.8, barbZ);
    this.scene.add(barbLight);

    // Barbearia Physics Collider
    this.physics.addBoxCollider(new THREE.Vector3(-13.5, 0, 78.0), new THREE.Vector3(-6.0, barbY + 5.5, 92.0), 'solid');

    // 2. Açougue Boi de Ouro (Z = 96.0 to 110.0, Center Z = 103.0)
    const acougueZ = 103.0;
    const acougueY = calcElevation(acougueZ);
    const acougueGeo = new THREE.BoxGeometry(7.5, 5.2, 14.0);
    const acougueMat = new THREE.MeshLambertMaterial({
      map: this.textures.createPaintedWall('#f8fafc', 2, 2)
    });
    const acougueMesh = new THREE.Mesh(acougueGeo, acougueMat);
    acougueMesh.position.set(-9.75, acougueY + 2.6, acougueZ);
    this.scene.add(acougueMesh);

    // Açougue Sign
    const acougueSignMat = new THREE.MeshBasicMaterial({ map: this.textures.createAcougueSign() });
    const acougueSign = new THREE.Mesh(new THREE.BoxGeometry(0.1, 1.2, 6.0), acougueSignMat);
    acougueSign.position.set(-5.95, acougueY + 3.8, acougueZ);
    this.scene.add(acougueSign);

    // Butcher Awning (Toldo listrado vermelho e branco)
    const toldoMat = new THREE.MeshLambertMaterial({ color: 0x991b1b });
    const toldoMesh = new THREE.Mesh(new THREE.BoxGeometry(1.6, 0.1, 7.0), toldoMat);
    toldoMesh.rotation.z = -0.25;
    toldoMesh.position.set(-5.2, acougueY + 3.0, acougueZ);
    this.scene.add(toldoMesh);

    // Refrigerated counter prop inside
    const counterMesh = new THREE.Mesh(
      new THREE.BoxGeometry(1.2, 1.0, 4.5),
      new THREE.MeshLambertMaterial({ color: 0xe2e8f0 })
    );
    counterMesh.position.set(-7.8, acougueY + 0.5, acougueZ);
    this.scene.add(counterMesh);

    // Açougue Physics Collider
    this.physics.addBoxCollider(new THREE.Vector3(-13.5, 0, 96.0), new THREE.Vector3(-6.0, acougueY + 5.5, 110.0), 'solid');

    // 3. Stepped Terraced Sobrados (West Flank)
    // Sobrado 1 (Ochre & Wood): Z = 114.0 to 126.0 (Center Z = 120.0)
    const sob1Z = 120.0;
    const sob1Y = calcElevation(sob1Z);
    const sob1Mesh = new THREE.Mesh(
      new THREE.BoxGeometry(7.5, 6.8, 12.0),
      new THREE.MeshLambertMaterial({ map: this.textures.createSobradoFacade('#d97706', '#1e293b') })
    );
    sob1Mesh.position.set(-9.75, sob1Y + 3.4, sob1Z);
    this.scene.add(sob1Mesh);

    const roof1 = new THREE.Mesh(new THREE.BoxGeometry(7.8, 0.4, 12.4), roofMat);
    roof1.position.set(-9.75, sob1Y + 6.9, sob1Z);
    this.scene.add(roof1);

    // Interactive Door for Sobrado 1
    const door1Pivot = new THREE.Group();
    door1Pivot.position.set(-6.0, sob1Y, sob1Z - 2.0);
    const door1Mesh = new THREE.Mesh(
      new THREE.BoxGeometry(0.08, 2.1, 1.1),
      new THREE.MeshLambertMaterial({ map: this.textures.createWoodenHouseDoorTexture() })
    );
    door1Mesh.position.set(0, 1.05, 0.55);
    door1Pivot.add(door1Mesh);
    this.scene.add(door1Pivot);

    const door1Collider = this.physics.addBoxCollider(
      new THREE.Vector3(-6.2, sob1Y, sob1Z - 2.6),
      new THREE.Vector3(-5.8, sob1Y + 2.2, sob1Z - 1.4),
      'solid'
    );
    this.interactiveDoors.push({
      id: 'sobrado_paula_120',
      name: 'SOBRADO RESIDENCIAL 120',
      position: new THREE.Vector3(-5.6, sob1Y + 1.1, sob1Z - 2.0),
      maxDist: 2.5,
      doorMesh: door1Pivot,
    ...
    this.physics.addBoxCollider(new THREE.Vector3(-13.5, 0, 114.0), new THREE.Vector3(-6.0, sob1Y + 7.2, 126.0), 'solid');

    // Sobrado 2 (Blue & White with Watchdog): Z = 128.0 to 140.0 (Center Z = 134.0)
    const sob2Z = 134.0;
    const sob2Y = calcElevation(sob2Z);
    const sob2Mesh = new THREE.Mesh(
      new THREE.BoxGeometry(7.5, 6.8, 12.0),
      new THREE.MeshLambertMaterial({ map: this.textures.createSobradoFacade('#0284c7', '#ffffff') })
    );
    sob2Mesh.position.set(-9.75, sob2Y + 3.4, sob2Z);
    this.scene.add(sob2Mesh);

    const roof2 = new THREE.Mesh(new THREE.BoxGeometry(7.8, 0.4, 12.4), roofMat);
    roof2.position.set(-9.75, sob2Y + 6.9, sob2Z);
    this.scene.add(roof2);

    // Front yard black iron gate
    const gateMat = new THREE.MeshLambertMaterial({ color: 0x18181b });
    const gate = new THREE.Mesh(new THREE.BoxGeometry(0.08, 1.8, 3.2), gateMat);
    gate.position.set(-6.0, sob2Y + 0.9, sob2Z + 2.5);
    this.scene.add(gate);

    // Warning Sign "CUIDADO COM O CÃO / BRAVO"
    const dogSignMat = new THREE.MeshBasicMaterial({ color: 0xfef08a });
    const dogSign = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.4, 0.8), dogSignMat);
    dogSign.position.set(-5.95, sob2Y + 1.4, sob2Z + 2.5);
    this.scene.add(dogSign);

    // Watchdog prop behind gate (Caramelo / Guard Dog)
    const dogGroup = new THREE.Group();
    const dogFurMat = new THREE.MeshLambertMaterial({ color: 0xc2782b });
    const dogTorso = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.4, 0.7), dogFurMat);
    dogTorso.position.set(0, 0.45, 0);
    dogGroup.add(dogTorso);
    const dogHead = new THREE.Mesh(new THREE.BoxGeometry(0.28, 0.3, 0.35), dogFurMat);
    dogHead.position.set(0, 0.72, 0.38);
    dogGroup.add(dogHead);
    const dogSnout = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.15, 0.22), new THREE.MeshLambertMaterial({ color: 0x1c1917 }));
    dogSnout.position.set(0, 0.65, 0.58);
    dogGroup.add(dogSnout);
    const earGeo = new THREE.ConeGeometry(0.08, 0.18, 4);
    const earL = new THREE.Mesh(earGeo, dogFurMat);
    earL.position.set(-0.12, 0.92, 0.32);
    dogGroup.add(earL);
    const earR = new THREE.Mesh(earGeo, dogFurMat);
    earR.position.set(0.12, 0.92, 0.32);
    dogGroup.add(earR);
    const dogTail = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 0.4, 6), dogFurMat);
    dogTail.rotation.x = -0.6;
    dogTail.position.set(0, 0.6, -0.42);
    dogGroup.add(dogTail);
    dogGroup.position.set(-6.8, sob2Y, sob2Z + 2.5);
    dogGroup.rotation.y = Math.PI / 2;
    this.scene.add(dogGroup);

    this.physics.addBoxCollider(new THREE.Vector3(-13.5, 0, 128.0), new THREE.Vector3(-6.0, sob2Y + 7.2, 140.0), 'solid');

    // Sobrado 3 (Colonial Sobrado): Z = 142.0 to 154.0 (Center Z = 148.0)
    const sob3Z = 148.0;
    const sob3Y = calcElevation(sob3Z);
    const sob3Mesh = new THREE.Mesh(
      new THREE.BoxGeometry(7.5, 6.8, 12.0),
      new THREE.MeshLambertMaterial({ map: this.textures.createSobradoFacade('#fef08a', '#334155') })
    );
    sob3Mesh.position.set(-9.75, sob3Y + 3.4, sob3Z);
    this.scene.add(sob3Mesh);

    const roof3 = new THREE.Mesh(new THREE.BoxGeometry(7.8, 0.4, 12.4), roofMat);
    roof3.position.set(-9.75, sob3Y + 6.9, sob3Z);
    this.scene.add(roof3);

    this.physics.addBoxCollider(new THREE.Vector3(-13.5, 0, 142.0), new THREE.Vector3(-6.0, sob3Y + 7.2, 154.0), 'solid');

    // =========================================================================
    // EAST SIDE OF PAULA FERREIRA (X: 9.0 to 16.5)
    // =========================================================================

    // 4. Hortifrúti da Ladeira (Z = 78.0 to 92.0, Center Z = 85.0)
    const hortiZ = 85.0;
    const hortiY = calcElevation(hortiZ);
    const hortiMesh = new THREE.Mesh(
      new THREE.BoxGeometry(7.5, 4.8, 14.0),
      new THREE.MeshLambertMaterial({ map: this.textures.createPaintedWall('#15803d', 2, 2) })
    );
    hortiMesh.position.set(12.75, hortiY + 2.4, hortiZ);
    this.scene.add(hortiMesh);

    // Hortifrúti Sign
    const hortiSignMat = new THREE.MeshBasicMaterial({ map: this.textures.createHortifrutiSign() });
    const hortiSign = new THREE.Mesh(new THREE.BoxGeometry(0.1, 1.2, 6.0), hortiSignMat);
    hortiSign.position.set(8.95, hortiY + 3.6, hortiZ);
    this.scene.add(hortiSign);

    // Green canvas awning projecting over sidewalk
    const hortiAwningMat = new THREE.MeshLambertMaterial({ color: 0x166534 });
    const hortiAwning = new THREE.Mesh(new THREE.BoxGeometry(1.6, 0.1, 7.0), hortiAwningMat);
    hortiAwning.rotation.z = 0.25;
    hortiAwning.position.set(8.2, hortiY + 2.9, hortiZ);
    this.scene.add(hortiAwning);

    // Sidewalk wooden fruit crate display stands
    const crateWoodMat = new THREE.MeshLambertMaterial({ color: 0x854d0e });
    [-2.0, 0.0, 2.0].forEach((offZ) => {
      const stand = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.7, 1.4), crateWoodMat);
      stand.position.set(8.5, hortiY + 0.35, hortiZ + offZ);
      this.scene.add(stand);

      const fruitMat = new THREE.MeshLambertMaterial({
        color: offZ < 0 ? 0x15803d : (offZ === 0 ? 0xf97316 : 0xfacc15)
      });
      const fruitGeo = new THREE.SphereGeometry(0.14, 8, 8);
      for (let fx = -0.2; fx <= 0.2; fx += 0.2) {
        for (let fz = -0.4; fz <= 0.4; fz += 0.25) {
          const fruit = new THREE.Mesh(fruitGeo, fruitMat);
          fruit.position.set(8.5 + fx, hortiY + 0.76, hortiZ + offZ + fz);
          this.scene.add(fruit);
        }
      }
    });

    this.physics.addBoxCollider(new THREE.Vector3(9.0, 0, 78.0), new THREE.Vector3(16.5, hortiY + 5.2, 92.0), 'solid');

    // 5. Pet Shop Au-Au Pirituba (Z = 96.0 to 110.0, Center Z = 103.0)
    const petZ = 103.0;
    const petY = calcElevation(petZ);
    const petMesh = new THREE.Mesh(
      new THREE.BoxGeometry(7.5, 4.8, 14.0),
      new THREE.MeshLambertMaterial({ map: this.textures.createPaintedWall('#0284c7', 2, 2) })
    );
    petMesh.position.set(12.75, petY + 2.4, petZ);
    this.scene.add(petMesh);

    // Pet Shop Sign
    const petSignMat = new THREE.MeshBasicMaterial({ map: this.textures.createPetShopAuAuSign() });
    const petSign = new THREE.Mesh(new THREE.BoxGeometry(0.1, 1.2, 6.0), petSignMat);
    petSign.position.set(8.95, petY + 3.6, petZ);
    this.scene.add(petSign);

    // Doghouse prop on sidewalk
    const doghouseMat = new THREE.MeshLambertMaterial({ color: 0x991b1b });
    const doghouse = new THREE.Mesh(new THREE.BoxGeometry(0.9, 0.8, 0.9), doghouseMat);
    doghouse.position.set(8.5, petY + 0.4, petZ - 2.5);
    this.scene.add(doghouse);

    const doghouseRoof = new THREE.Mesh(new THREE.ConeGeometry(0.7, 0.4, 4), new THREE.MeshLambertMaterial({ color: 0x18181b }));
    doghouseRoof.rotation.y = Math.PI / 4;
    doghouseRoof.position.set(8.5, petY + 0.95, petZ - 2.5);
    this.scene.add(doghouseRoof);

    // Water bowl
    const bowl = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.14, 0.1, 12), new THREE.MeshLambertMaterial({ color: 0x38bdf8 }));
    bowl.position.set(8.5, petY + 0.05, petZ - 1.6);
    this.scene.add(bowl);

    // Friendly Golden Retriever prop sitting outside
    const petDogGroup = new THREE.Group();
    const goldenMat = new THREE.MeshLambertMaterial({ color: 0xf59e0b });
    const petDogBody = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.45, 0.6), goldenMat);
    petDogBody.position.set(0, 0.35, 0);
    petDogGroup.add(petDogBody);
    const petDogHead = new THREE.Mesh(new THREE.BoxGeometry(0.28, 0.28, 0.32), goldenMat);
    petDogHead.position.set(0, 0.65, 0.3);
    petDogGroup.add(petDogHead);
    const petDogSnout = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.12, 0.18), new THREE.MeshLambertMaterial({ color: 0x1c1917 }));
    petDogSnout.position.set(0, 0.58, 0.48);
    petDogGroup.add(petDogSnout);
    petDogGroup.position.set(8.5, petY, petZ - 0.8);
    petDogGroup.rotation.y = -Math.PI / 2;
    this.scene.add(petDogGroup);

    this.physics.addBoxCollider(new THREE.Vector3(9.0, 0, 96.0), new THREE.Vector3(16.5, petY + 5.2, 110.0), 'solid');

    // 6. Bar da Ladeira (Sinuca & Dominó) (Z = 114.0 to 128.0, Center Z = 121.0)
    const barZ = 121.0;
    const barY = calcElevation(barZ);
    const barMesh = new THREE.Mesh(
      new THREE.BoxGeometry(7.5, 5.0, 14.0),
      new THREE.MeshLambertMaterial({ map: this.textures.createPaintedWall('#0369a1', 2, 2) })
    );
    barMesh.position.set(12.75, barY + 2.5, barZ);
    this.scene.add(barMesh);

    // Bar da Ladeira Sign
    const barSignMat = new THREE.MeshBasicMaterial({ map: this.textures.createBotecoLadeiraSign() });
    const barSign = new THREE.Mesh(new THREE.BoxGeometry(0.1, 1.2, 6.0), barSignMat);
    barSign.position.set(8.95, barY + 3.8, barZ);
    this.scene.add(barSign);

    // Yellow boteco awning overhead
    const barAwningMat = new THREE.MeshLambertMaterial({ color: 0xeab308 });
    const barAwning = new THREE.Mesh(new THREE.BoxGeometry(1.6, 0.1, 7.0), barAwningMat);
    barAwning.rotation.z = 0.25;
    barAwning.position.set(8.2, barY + 3.0, barZ);
    this.scene.add(barAwning);

    // Sidewalk Domino Scene
    const dTableGeo = new THREE.BoxGeometry(1.1, 0.08, 1.1);
    const dTableMat = new THREE.MeshLambertMaterial({ color: 0x78350f });
    const dTable = new THREE.Mesh(dTableGeo, dTableMat);
    dTable.position.set(8.3, barY + 0.72, barZ);
    this.scene.add(dTable);

    const dLegGeo = new THREE.CylinderGeometry(0.04, 0.04, 0.72, 8);
    const dLegMat = new THREE.MeshLambertMaterial({ color: 0x18181b });
    [-0.45, 0.45].forEach(lx => {
      [-0.45, 0.45].forEach(lz => {
        const leg = new THREE.Mesh(dLegGeo, dLegMat);
        leg.position.set(8.3 + lx, barY + 0.36, barZ + lz);
        this.scene.add(leg);
      });
    });

    // White domino tiles arranged on the table
    const dominoTileMat = new THREE.MeshBasicMaterial({ color: 0xf8fafc });
    const tileGeo = new THREE.BoxGeometry(0.06, 0.02, 0.12);
    for (let t = -0.3; t <= 0.3; t += 0.12) {
      const tile = new THREE.Mesh(tileGeo, dominoTileMat);
      tile.position.set(8.3 + (Math.sin(t * 10) * 0.1), barY + 0.77, barZ + t);
      tile.rotation.y = t * 1.5;
      this.scene.add(tile);
    }

    // Yellow ice bucket and brown beer bottle on table
    const iceBucket = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.09, 0.16, 10), new THREE.MeshLambertMaterial({ color: 0xfacc15 }));
    iceBucket.position.set(8.65, barY + 0.82, barZ + 0.35);
    this.scene.add(iceBucket);
    const beerBottle = new THREE.Mesh(new THREE.CylinderGeometry(0.035, 0.035, 0.24, 8), new THREE.MeshLambertMaterial({ color: 0x451a03 }));
    beerBottle.position.set(8.65, barY + 0.92, barZ + 0.35);
    this.scene.add(beerBottle);

    // Red plastic boteco chairs around table
    const chairMat = new THREE.MeshLambertMaterial({ color: 0xdc2626 });
    [-0.7, 0.7].forEach((cz) => {
      const chair = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.45, 0.5), chairMat);
      chair.position.set(8.3, barY + 0.45, barZ + cz);
      this.scene.add(chair);

      // Retired senhorzinho seated character
      const humanGroup = new THREE.Group();
      const skinMat = new THREE.MeshLambertMaterial({ color: 0xd4a373 });
      const shirtMat = new THREE.MeshLambertMaterial({ color: cz < 0 ? 0x0284c7 : 0x16a34a });
      const hatMat = new THREE.MeshLambertMaterial({ color: 0xfef08a });

      const torso = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.45, 0.25), shirtMat);
      ...
      const hat = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.22, 0.04, 12), hatMat);
      hat.position.set(0, 1.08, 0);
      humanGroup.add(hat);

      humanGroup.position.set(8.3, barY + 0.2, barZ + cz);
      humanGroup.rotation.y = cz < 0 ? 0 : Math.PI;
      this.scene.add(humanGroup);
    });

    this.physics.addBoxCollider(new THREE.Vector3(9.0, 0, 114.0), new THREE.Vector3(16.5, barY + 5.5, 128.0), 'solid');

    // 7. Stepped Historic Sobrados (East Flank)
    // Sobrado East 1 (Pastel Rose): Z = 130.0 to 142.0 (Center Z = 136.0)
    const sobE1Z = 136.0;
    const sobE1Y = calcElevation(sobE1Z);
    const sobE1Mesh = new THREE.Mesh(
      new THREE.BoxGeometry(7.5, 6.8, 12.0),
      new THREE.MeshLambertMaterial({ map: this.textures.createSobradoFacade('#fda4af', '#475569') })
    );
    sobE1Mesh.position.set(12.75, sobE1Y + 3.4, sobE1Z);
    this.scene.add(sobE1Mesh);

    const roofE1 = new THREE.Mesh(new THREE.BoxGeometry(7.8, 0.4, 12.4), roofMat);
    roofE1.position.set(12.75, sobE1Y + 6.9, sobE1Z);
    this.scene.add(roofE1);

    this.physics.addBoxCollider(new THREE.Vector3(9.0, 0, 130.0), new THREE.Vector3(16.5, sobE1Y + 7.2, 142.0), 'solid');

    // Sobrado East 2 (Colonial Yellow & White): Z = 144.0 to 154.0 (Center Z = 149.0)
    const sobE2Z = 149.0;
    const sobE2Y = calcElevation(sobE2Z);
    const sobE2Mesh = new THREE.Mesh(
      new THREE.BoxGeometry(7.5, 6.8, 10.0),
      new THREE.MeshLambertMaterial({ map: this.textures.createColonialWall('#e8c96b', 2, 2) })
    );
    sobE2Mesh.position.set(12.75, sobE2Y + 3.4, sobE2Z);
    this.scene.add(sobE2Mesh);

    const roofE2 = new THREE.Mesh(new THREE.BoxGeometry(7.8, 0.4, 10.4), roofMat);
    roofE2.position.set(12.75, sobE2Y + 6.9, sobE2Z);
    this.scene.add(roofE2);

    this.physics.addBoxCollider(new THREE.Vector3(9.0, 0, 144.0), new THREE.Vector3(16.5, sobE2Y + 7.2, 154.0), 'solid');
  }
  ...
  // 21.7 Continuous Commerces, Services & Authentic Establishments along Av. Gen. Edgar Facó
  buildEdgarFaccoExtendedCommerces() {
    const chromeMat = new THREE.MeshLambertMaterial({ color: 0xd4d4d8 });
    const tireMat = new THREE.MeshLambertMaterial({ color: 0x09090b });

    // =========================================================================
    // WEST EDGAR FACÓ (X: -58.0 to -20.0)
    // =========================================================================

    // 1. Casa do Norte Asa Branca (South side: X = -34.0 to -20.5, Z = 37.0 to 47.0)
    const cNorteGeo = new THREE.BoxGeometry(13.5, 5.5, 10.0);
    const cNorteMat = new THREE.MeshLambertMaterial({
      map: this.textures.createPaintedWall('#78350f', 2, 2)
    });
    const cNorteMesh = new THREE.Mesh(cNorteGeo, cNorteMat);
    cNorteMesh.position.set(-27.25, 2.75, 42.0);
    this.scene.add(cNorteMesh);

    // Signboard
    const cNorteSignMat = new THREE.MeshBasicMaterial({ map: this.textures.createCasaDoNorteSign() });
    const cNorteSign = new THREE.Mesh(new THREE.BoxGeometry(6.0, 1.2, 0.1), cNorteSignMat);
    cNorteSign.position.set(-27.25, 4.2, 36.95);
    this.scene.add(cNorteSign);

    // Rustic barrel table and stools on sidewalk
    const barrelMat = new THREE.MeshLambertMaterial({ color: 0x92400e });
    const barrel = new THREE.Mesh(new THREE.CylinderGeometry(0.4, 0.45, 0.85, 12), barrelMat);
    barrel.position.set(-23.5, 0.425, 35.5);
    this.scene.add(barrel);

    const stoolMat = new THREE.MeshLambertMaterial({ color: 0x451a03 });
    [-0.55, 0.55].forEach(sx => {
      const stool = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.2, 0.5, 8), stoolMat);
      stool.position.set(-23.5 + sx, 0.25, 35.5);
      this.scene.add(stool);
    });

    this.physics.addBoxCollider(new THREE.Vector3(-34.0, 0, 37.0), new THREE.Vector3(-20.5, 6.0, 47.0), 'solid');

    // 2. Depósito Pirituba - Materiais de Construção (North side: X = -48.0 to -36.0, Z = -10.0 to 2.0)
    const depGeo = new THREE.BoxGeometry(12.0, 5.8, 12.0);
    const depMat = new THREE.MeshLambertMaterial({
      map: this.textures.createPaintedWall('#475569', 2, 2)
    });
    const depMesh = new THREE.Mesh(depGeo, depMat);
    depMesh.position.set(-42.0, 2.9, -4.0);
    this.scene.add(depMesh);

    // Depósito Signboard
    const depSignTex = this.textures.createStoreSign('DEPÓSITO PIRITUBA', '#1e293b', '#facc15');
    const depSign = new THREE.Mesh(new THREE.BoxGeometry(6.0, 1.2, 0.1), new THREE.MeshBasicMaterial({ map: depSignTex }));
    depSign.position.set(-42.0, 4.5, 2.05);
    this.scene.add(depSign);

    // Yard props: Stacks of reddish-orange bricks (tijolo baiano)
    const brickMat = new THREE.MeshLambertMaterial({ map: this.textures.createTijoloBaiano(2, 2) });
    const brickStack = new THREE.Mesh(new THREE.BoxGeometry(1.8, 1.2, 1.8), brickMat);
    brickStack.position.set(-37.5, 0.6, 3.2);
    this.scene.add(brickStack);

    // Cone pile of construction sand
    const sandMat = new THREE.MeshLambertMaterial({ color: 0xd4a373 });
    const sandPile = new THREE.Mesh(new THREE.ConeGeometry(1.2, 0.9, 12), sandMat);
    sandPile.position.set(-45.5, 0.45, 3.2);
    this.scene.add(sandPile);

    this.physics.addBoxCollider(new THREE.Vector3(-48.0, 0, -10.0), new THREE.Vector3(-36.0, 6.0, 2.0), 'solid');

    // 3. Oficina de Motos Grau & Corte (North side: X = -58.0 to -48.0, Z = -10.0 to 2.0)
    const motoGeo = new THREE.BoxGeometry(10.0, 5.0, 12.0);
    const motoMat = new THREE.MeshLambertMaterial({
      map: this.textures.createPaintedWall('#18181b', 2, 2)
    });
    const motoMesh = new THREE.Mesh(motoGeo, motoMat);
    motoMesh.position.set(-53.0, 2.5, -4.0);
    this.scene.add(motoMesh);

    // Roll-up steel garage doors
    const steelDoorMat = new THREE.MeshLambertMaterial({ map: this.textures.createPortaAco('#334155') });
    const motoDoor = new THREE.Mesh(new THREE.BoxGeometry(4.0, 3.5, 0.1), steelDoorMat);
    motoDoor.position.set(-53.0, 1.75, 2.02);
    this.scene.add(motoDoor);

    // Neon motorcycle shop sign
    const motoSignTex = this.textures.createStoreSign('GRAU & CORTE MOTOS', '#09090b', '#22c55e');
    const motoSign = new THREE.Mesh(new THREE.BoxGeometry(5.0, 1.0, 0.1), new THREE.MeshBasicMaterial({ map: motoSignTex }));
    motoSign.position.set(-53.0, 4.0, 2.05);
    this.scene.add(motoSign);

    // Motorcycle prop parked outside
    const bikeGroup = new THREE.Group();
    const wheelGeo = new THREE.CylinderGeometry(0.28, 0.28, 0.1, 12);
    const frontWheel = new THREE.Mesh(wheelGeo, tireMat);
    frontWheel.rotation.z = Math.PI / 2;
    frontWheel.position.set(0.6, 0.28, 0);
    bikeGroup.add(frontWheel);
    const rearWheel = new THREE.Mesh(wheelGeo, tireMat);
    rearWheel.rotation.z = Math.PI / 2;
    rearWheel.position.set(-0.6, 0.28, 0);
    bikeGroup.add(rearWheel);
    const bikeFrame = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.3, 0.2), new THREE.MeshLambertMaterial({ color: 0xdc2626 }));
    bikeFrame.position.set(0, 0.55, 0);
    bikeGroup.add(bikeFrame);
    const handlebar = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.05, 0.6), chromeMat);
    handlebar.position.set(0.4, 0.75, 0);
    bikeGroup.add(handlebar);
    bikeGroup.position.set(-51.0, 0, 3.2);
    bikeGroup.rotation.y = -0.3;
    this.scene.add(bikeGroup);

    this.physics.addBoxCollider(new THREE.Vector3(-58.0, 0, -10.0), new THREE.Vector3(-48.0, 5.5, 2.0), 'solid');

    // =========================================================================
    // EAST EDGAR FACÓ (X: 57.0 to 135.0)
    // =========================================================================

    // 4. Lotérica Pirituba (South side: X = 57.0 to 68.0, Z = 37.0 to 47.0)
    const lotGeo = new THREE.BoxGeometry(11.0, 5.2, 10.0);
    const lotMat = new THREE.MeshLambertMaterial({
      map: this.textures.createPaintedWall('#f8fafc', 2, 2)
    });
    const lotMesh = new THREE.Mesh(lotGeo, lotMat);
    lotMesh.position.set(62.5, 2.6, 42.0);
    this.scene.add(lotMesh);

    // Lotérica Signboard
    const lotSignMat = new THREE.MeshBasicMaterial({ map: this.textures.createLotericaSign() });
    const lotSign = new THREE.Mesh(new THREE.BoxGeometry(5.5, 1.2, 0.1), lotSignMat);
    lotSign.position.set(62.5, 4.0, 36.95);
    this.scene.add(lotSign);

    // Armored glass counter pass-through on facade
    const glassCounter = new THREE.Mesh(
      new THREE.BoxGeometry(3.0, 1.5, 0.1),
      new THREE.MeshLambertMaterial({ color: 0x38bdf8, transparent: true, opacity: 0.6 })
    );
    glassCounter.position.set(62.5, 1.8, 36.96);
    this.scene.add(glassCounter);

    // Yellow queue stanchions on sidewalk
    const stanchionMat = new THREE.MeshLambertMaterial({ color: 0xfacc15 });
    [-1.2, 0, 1.2].forEach(ox => {
      const post = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 0.9, 8), stanchionMat);
      post.position.set(62.5 + ox, 0.45, 35.8);
      this.scene.add(post);
    });

    this.physics.addBoxCollider(new THREE.Vector3(57.0, 0, 37.0), new THREE.Vector3(68.0, 5.5, 47.0), 'solid');

    // 5. Pastelaria do Beto: Walk-in Salon with Dining Tables, Estufa, Moenda de Caldo & Seating
    this.buildWalkablePastelaria(74.5, 0, 42.5);

    // 6. Bar & Petiscaria Cantinho do Peixe (North side: X = 96.0 to 112.0, Z = -8.0 to 2.0)
    const peixeGeo = new THREE.BoxGeometry(16.0, 5.5, 10.0);
    const peixeMat = new THREE.MeshLambertMaterial({
      map: this.textures.createPaintedWall('#0284c7', 2, 2)
    });
    const peixeMesh = new THREE.Mesh(peixeGeo, peixeMat);
    peixeMesh.position.set(104.0, 2.75, -3.0);
    this.scene.add(peixeMesh);

    // Signboard
    const peixeSignMat = new THREE.MeshBasicMaterial({ map: this.textures.createBarPeixeSign() });
    const peixeSign = new THREE.Mesh(new THREE.BoxGeometry(6.5, 1.2, 0.1), peixeSignMat);
    peixeSign.position.set(104.0, 4.2, 2.05);
    this.scene.add(peixeSign);

    // Outdoor Dining Veranda: 3 tables with blue parasols
    const tableGeo = new THREE.CylinderGeometry(0.55, 0.55, 0.06, 12);
    const tableMat = new THREE.MeshLambertMaterial({ color: 0x0284c7 });
    const umbrellaMat = new THREE.MeshLambertMaterial({ color: 0x0369a1 });
    const umbrellaGeo = new THREE.ConeGeometry(1.2, 0.4, 8);

    [-4.0, 0, 4.0].forEach(ox => {
      const tPos = new THREE.Vector3(104.0 + ox, 0.72, 3.8);
      const tbl = new THREE.Mesh(tableGeo, tableMat);
      tbl.position.copy(tPos);
      this.scene.add(tbl);

      const pole = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 2.2, 8), chromeMat);
      pole.position.set(tPos.x, 1.1, tPos.z);
      this.scene.add(pole);

      const umbrella = new THREE.Mesh(umbrellaGeo, umbrellaMat);
      umbrella.position.set(tPos.x, 2.2, tPos.z);
      this.scene.add(umbrella);

      [-0.7, 0.7].forEach(cz => {
        const chr = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.45, 0.4), new THREE.MeshLambertMaterial({ color: 0xffffff }));
        chr.position.set(tPos.x, 0.225, tPos.z + cz);
        this.scene.add(chr);
      });
    });

    this.physics.addBoxCollider(new THREE.Vector3(96.0, 0, -8.0), new THREE.Vector3(112.0, 6.0, 2.0), 'solid');

    // 7. Ponto de Táxi Pirituba (North sidewalk: X = 120.0 to 130.0, Z = 4.0 to 8.0)
    const taxiSignMat = new THREE.MeshBasicMaterial({ color: 0x0284c7 });
    const taxiPlate = new THREE.Mesh(new THREE.CylinderGeometry(0.4, 0.4, 0.05, 16), taxiSignMat);
    taxiPlate.rotation.x = Math.PI / 2;
    taxiPlate.position.set(122.0, 2.8, 6.0);
    this.scene.add(taxiPlate);

    const taxiPole = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 2.8, 8), chromeMat);
    taxiPole.position.set(122.0, 1.4, 6.0);
    this.scene.add(taxiPole);

    // Chevrolet Spin Taxi Prop parked at curb
    const taxiCar = new THREE.Group();
    const taxiBodyMat = new THREE.MeshLambertMaterial({ color: 0xf8fafc });
    const carBody = new THREE.Mesh(new THREE.BoxGeometry(3.6, 0.9, 1.6), taxiBodyMat);
    carBody.position.set(0, 0.65, 0);
    taxiCar.add(carBody);

    const carCabin = new THREE.Mesh(new THREE.BoxGeometry(2.0, 0.7, 1.4), new THREE.MeshLambertMaterial({ color: 0x334155 }));
    carCabin.position.set(-0.2, 1.4, 0);
    taxiCar.add(carCabin);

    // SPTrans Red & Blue Stripe decal on side doors
    const stripeBlue = new THREE.Mesh(new THREE.BoxGeometry(2.4, 0.06, 1.62), new THREE.MeshBasicMaterial({ color: 0x0284c7 }));
    stripeBlue.position.set(0, 0.68, 0);
    taxiCar.add(stripeBlue);
    const stripeRed = new THREE.Mesh(new THREE.BoxGeometry(2.4, 0.04, 1.62), new THREE.MeshBasicMaterial({ color: 0xdc2626 }));
    stripeRed.position.set(0, 0.62, 0);
    taxiCar.add(stripeRed);

    // Lighted Taxi Roof Sign
    const taxiRoofSign = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.18, 0.2), new THREE.MeshBasicMaterial({ color: 0xfacc15 }));
    taxiRoofSign.position.set(-0.2, 1.84, 0);
    taxiCar.add(taxiRoofSign);

    // 4 Wheels
    [-1.1, 1.1].forEach(wx => {
      [-0.8, 0.8].forEach(wz => {
        const whl = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 0.3, 0.2, 12), tireMat);
        whl.rotation.x = Math.PI / 2;
        whl.position.set(wx, 0.3, wz);
        taxiCar.add(whl);
      });
    });

    taxiCar.position.set(126.0, 0, 6.2);
    this.scene.add(taxiCar);
    this.physics.addBoxCollider(new THREE.Vector3(124.0, 0, 5.2), new THREE.Vector3(128.0, 2.0, 7.2), 'solid');
  }
  ...
  // =========================================================================
  // 24.6 Av. Ministro Petrônio Portela Further South Extension (Z = 178 to 330)
  // Hillside Igrejinha & Praça, Colégio Wellington, Mercado Pyrituba,
  // Restaurante Amigos do Picuí, Drogaria & Farmácia Petrônio, Praça da Colina
  // =========================================================================
  buildPetronioPortelaFurtherSouthExtension() {
    const roadMat = new THREE.MeshLambertMaterial({
      map: this.textures.createAsfalto(4, 30)
    });
    const sideMat = new THREE.MeshLambertMaterial({
      map: this.textures.createCalcadaPaulista(4, 30)
    });
    const yellowStripeMat = new THREE.MeshBasicMaterial({ color: 0xfacc15 });
    const whiteStripeMat = new THREE.MeshBasicMaterial({ color: 0xffffff });
    const curbMat = new THREE.MeshLambertMaterial({ color: 0x94a3b8 });
    const grassMat = new THREE.MeshLambertMaterial({ color: 0x2e7d32 });
    const cobblestoneMat = new THREE.MeshLambertMaterial({
      map: this.textures.createCalcadaPaulista(2, 2)
    });
    const colonialWallMat = new THREE.MeshLambertMaterial({
      map: this.textures.createPaintedWall('#fdfbf7', 2, 2)
    });
    const terracottaRoofMat = new THREE.MeshLambertMaterial({
      map: this.textures.createTelhasColoniais()
    });
    const woodMat = new THREE.MeshLambertMaterial({ color: 0x5c2c16 });
    const brickMat = new THREE.MeshLambertMaterial({
      map: this.textures.createTijoloBaiano(3, 3)
    });
    ...
    // 1. Roadway Asphalt (X = 137.0 to 153.0, Z = 178.0 to 330.0, Length = 152m)
    const roadGeo = new THREE.PlaneGeometry(16.0, 152.0);
    const roadMesh = new THREE.Mesh(roadGeo, roadMat);
    roadMesh.rotation.x = -Math.PI / 2;
    roadMesh.position.set(145.0, 0.01, 254.0);
    this.scene.add(roadMesh);

    // Center dashed double yellow line (Faixa Amarela Dupla Pontilhada)
    for (let z = 180.0; z < 328.0; z += 5.0) {
      [-0.15, 0.15].forEach(dx => {
        const stripe = new THREE.Mesh(new THREE.PlaneGeometry(0.12, 2.8), yellowStripeMat);
        stripe.rotation.x = -Math.PI / 2;
        stripe.position.set(145.0 + dx, 0.016, z);
    ...
    // Pedestrian crosswalks at Z = 210.0, 270.0, 315.0
    [210.0, 270.0, 315.0].forEach(crossZ => {
      for (let x = 137.6; x <= 152.4; x += 1.6) {
        const stripe = new THREE.Mesh(new THREE.PlaneGeometry(0.8, 4.0), whiteStripeMat);
        stripe.rotation.x = -Math.PI / 2;
        stripe.position.set(x, 0.018, crossZ);
    ...
    // 2. Sidewalks (West: X = 131 to 137, East: X = 153 to 159)
    const westSide = new THREE.Mesh(new THREE.BoxGeometry(6.0, 0.25, 152.0), sideMat);
    westSide.position.set(134.0, 0.125, 254.0);
    this.scene.add(westSide);
    this.physics.addBoxCollider(
      new THREE.Vector3(131.0, 0, 178.0),
      new THREE.Vector3(137.0, 0.25, 330.0),
      'curb'
    );

    const eastSide = new THREE.Mesh(new THREE.BoxGeometry(6.0, 0.25, 152.0), sideMat);
    eastSide.position.set(156.0, 0.125, 254.0);
    this.scene.add(eastSide);
    this.physics.addBoxCollider(
      new THREE.Vector3(153.0, 0, 178.0),
      new THREE.Vector3(159.0, 0.25, 330.0),
      'curb'
    );

    // 3. Concrete Utility Poles with Street Lights along Petrônio Portela
    [195.0, 222.0, 250.0, 278.0, 305.0, 325.0].forEach((pz, idx) => {
      this.buildUtilityPole(136.5, 0.25, pz, idx % 2 === 0);
      this.buildUtilityPole(153.5, 0.25, pz + 12.0, idx % 2 === 1);
    });
    ...
    // -------------------------------------------------------------------------
    {
      const churchHillGroup = new THREE.Group();
      churchHillGroup.name = 'igrejinhaMorroGroup';

      // Hill terrain mound (base Y = 0 to 4.5)
      const hillBase = new THREE.Mesh(
        new THREE.BoxGeometry(32.0, 4.5, 40.0),
        grassMat
      );
      hillBase.position.set(176.0, 2.25, 205.0);
      churchHillGroup.add(hillBase);

      // Plateau plaza surface with Portuguese cobblestone (Y = 4.5)
      const plazaFloor = new THREE.Mesh(
        new THREE.PlaneGeometry(30.0, 38.0),
        cobblestoneMat
      );
      plazaFloor.rotation.x = -Math.PI / 2;
      plazaFloor.position.set(176.0, 4.51, 205.0);
      churchHillGroup.add(plazaFloor);

      // Stone retaining wall facing avenue
      const wallFront = new THREE.Mesh(
        new THREE.BoxGeometry(1.2, 4.5, 40.0),
        curbMat
      );
      wallFront.position.set(160.0, 2.25, 205.0);
      churchHillGroup.add(wallFront);

      // Plateau Walkable Collider
      this.physics.addBoxCollider(
        new THREE.Vector3(160.0, 0, 185.0),
        new THREE.Vector3(192.0, 4.5, 225.0),
        'solid'
      );

      // Stone Stairs / Escadaria leading from sidewalk (X = 159, Z = 202 to 206) up to Y = 4.5
      const numSteps = 10;
      const stepRise = 4.5 / numSteps;
      for (let i = 0; i < numSteps; i++) {
        const stepY = (i + 1) * stepRise;
        const stepX = 160.0 + (i * 0.7);
        const step = new THREE.Mesh(
          new THREE.BoxGeometry(1.2, stepY, 4.0),
          curbMat
        );
        step.position.set(stepX, stepY / 2, 204.0);
        churchHillGroup.add(step);

        // Stepped walk collider so player climbs smoothly
        this.physics.addBoxCollider(
          new THREE.Vector3(stepX - 0.6, 0, 202.0),
          new THREE.Vector3(stepX + 0.6, stepY, 206.0),
          'curb'
        );
      }

      // Overlook railing along West edge of plateau (X = 160.5, Z = 186 to 224 except stairs)
      const railMat = new THREE.MeshLambertMaterial({ color: 0x1f2937 });
      const railNorth = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.9, 16.0), railMat);
      railNorth.position.set(160.5, 4.95, 193.0);
      churchHillGroup.add(railNorth);
      const railSouth = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.9, 16.0), railMat);
      railSouth.position.set(160.5, 4.95, 215.0);
      churchHillGroup.add(railSouth);

      // Park benches and ornamental lanterns on the church plaza
      [-6.0, 6.0].forEach(dz => {
        const bench = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.5, 2.2), woodMat);
        bench.position.set(164.0, 4.75, 205.0 + dz);
        churchHillGroup.add(bench);
      });

      // Colonial Chapel / Igrejinha (at X = 180.0, Y = 4.5, Z = 205.0)
      // Main Nave
      const chapelNave = new THREE.Mesh(
        new THREE.BoxGeometry(14.0, 7.0, 16.0),
        colonialWallMat
      );
      chapelNave.position.set(180.0, 4.5 + 3.5, 205.0);
      churchHillGroup.add(chapelNave);

      // Peaked Tile Roof
      const roofPeak = new THREE.Mesh(
        new THREE.ConeGeometry(9.0, 3.5, 4),
        terracottaRoofMat
      );
      roofPeak.rotation.y = Math.PI / 4;
      roofPeak.position.set(180.0, 4.5 + 7.0 + 1.75, 205.0);
      churchHillGroup.add(roofPeak);

      // Front Facade Portico (West facing towards avenue)
      const facade = new THREE.Mesh(
        new THREE.BoxGeometry(1.5, 9.0, 10.0),
        colonialWallMat
      );
      facade.position.set(172.5, 4.5 + 4.5, 205.0);
      churchHillGroup.add(facade);

      // Wooden Double Church Doors
      const churchDoor = new THREE.Mesh(
        new THREE.BoxGeometry(0.2, 3.2, 2.4),
        woodMat
      );
      churchDoor.position.set(171.7, 4.5 + 1.6, 205.0);
      churchHillGroup.add(churchDoor);

      // Chapel Sign Plaque
      const plaqueMat = new THREE.MeshBasicMaterial({
        map: this.textures.createIgrejinhaMorroSign()
      });
      const plaque = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.8, 1.8), plaqueMat);
      plaque.position.set(171.6, 4.5 + 3.6, 202.2);
      churchHillGroup.add(plaque);

      // Bell Tower / Campanário (Top of facade)
      const tower = new THREE.Mesh(
        new THREE.BoxGeometry(3.0, 4.0, 3.0),
        colonialWallMat
      );
      tower.position.set(173.0, 4.5 + 10.5, 205.0);
      churchHillGroup.add(tower);

      // Brass Bell
      const bell = new THREE.Mesh(
        new THREE.CylinderGeometry(0.35, 0.55, 0.8, 12),
        new THREE.MeshLambertMaterial({ color: 0xd97706 })
      );
      bell.position.set(173.0, 4.5 + 10.5, 205.0);
      churchHillGroup.add(bell);

      // Wooden Cross on Tower Peak
      const crossMat = new THREE.MeshLambertMaterial({ color: 0x78350f });
      const crossV = new THREE.Mesh(new THREE.BoxGeometry(0.18, 1.8, 0.18), crossMat);
      crossV.position.set(173.0, 4.5 + 13.0, 205.0);
      churchHillGroup.add(crossV);
      const crossH = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.18, 1.0), crossMat);
      crossH.position.set(173.0, 4.5 + 13.4, 205.0);
      churchHillGroup.add(crossH);

      // Chapel Solid Colliders
      this.physics.addBoxCollider(
        new THREE.Vector3(172.0, 4.5, 196.0),
        new THREE.Vector3(188.0, 18.0, 214.0),
        'solid'
      );

      this.scene.add(churchHillGroup);
    }

    // -------------------------------------------------------------------------
    // 5. EAST SIDE: COLÉGIO WELLINGTON (Z = 230 to 265, X = 160 to 188)
    // -------------------------------------------------------------------------
    {
      const schoolGroup = new THREE.Group();
      schoolGroup.name = 'colegioWellingtonGroup';

      const schoolBodyMat = new THREE.MeshLambertMaterial({
        map: this.textures.createPaintedWall('#f1f5f9', 4, 3)
      });
      const schoolNavyMat = new THREE.MeshLambertMaterial({ color: 0x1e3a8a });

      // Main School Building (X = 174, Z = 254)
      const schoolBody = new THREE.Mesh(new THREE.BoxGeometry(22.0, 9.0, 20.0), schoolBodyMat);
      schoolBody.position.set(174.0, 4.5, 254.0);
      schoolGroup.add(schoolBody);

      // Navy Blue Base Skirting
      const schoolBase = new THREE.Mesh(new THREE.BoxGeometry(22.2, 1.2, 20.2), schoolNavyMat);
      schoolBase.position.set(174.0, 0.6, 254.0);
      schoolGroup.add(schoolBase);

      // Classroom Windows on West Facade
      for (let wy = 3.2; wy <= 6.8; wy += 3.2) {
        for (let wz = 247.0; wz <= 261.0; wz += 4.5) {
          const win = new THREE.Mesh(new THREE.BoxGeometry(0.1, 1.8, 2.8), windowGlassMat);
          win.position.set(162.9, wy, wz);
          schoolGroup.add(win);
        }
      }

      // Large Official Signboard
      const signMat = new THREE.MeshBasicMaterial({
        map: this.textures.createColegioWellingtonSign()
      });
      const schoolSign = new THREE.Mesh(new THREE.BoxGeometry(0.2, 2.5, 10.0), signMat);
      schoolSign.position.set(162.8, 6.5, 254.0);
      schoolGroup.add(schoolSign);

      // Entrance Portico / Reception Porch
      const porticoRoof = new THREE.Mesh(new THREE.BoxGeometry(3.5, 0.3, 4.5), schoolNavyMat);
      porticoRoof.position.set(161.2, 3.2, 248.0);
      schoolGroup.add(porticoRoof);

      // Multi-sports court (Quadra Poliesportiva, Z = 232 to 244, X = 162 to 184)
      const courtFloorMat = new THREE.MeshLambertMaterial({ color: 0x0284c7 });
      const courtFloor = new THREE.Mesh(new THREE.PlaneGeometry(20.0, 12.0), courtFloorMat);
      courtFloor.rotation.x = -Math.PI / 2;
      courtFloor.position.set(173.0, 0.26, 238.0);
      schoolGroup.add(courtFloor);

      // Court Green Fence
      const fenceMat = new THREE.MeshLambertMaterial({ color: 0x15803d });
      const fenceW = new THREE.Mesh(new THREE.BoxGeometry(0.1, 2.8, 12.0), fenceMat);
      fenceW.position.set(162.0, 1.4, 238.0);
      schoolGroup.add(fenceW);

      // Basketball backboard & hoop
      const hoopPole = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 3.8, 6), curbMat);
      hoopPole.position.set(182.5, 1.9, 238.0);
      schoolGroup.add(hoopPole);
      const backboard = new THREE.Mesh(new THREE.BoxGeometry(0.1, 1.0, 1.6), new THREE.MeshBasicMaterial({ color: 0xffffff }));
      backboard.position.set(182.4, 3.2, 238.0);
      schoolGroup.add(backboard);

      this.scene.add(schoolGroup);

      this.physics.addBoxCollider(
        new THREE.Vector3(162.0, 0, 230.0),
        new THREE.Vector3(188.0, 10.0, 265.0),
        'solid'
      );
    }

    // -------------------------------------------------------------------------
    // 6. EAST SIDE: MERCADO MUNICIPAL DE PYRITUBA (Z = 270 to 300, X = 160 to 188)
    // -------------------------------------------------------------------------
    {
      const marketGroup = new THREE.Group();
      marketGroup.name = 'mercadoPyritubaGroup';

      // Brick Hall Base & Stucco Upper
      const marketBase = new THREE.Mesh(new THREE.BoxGeometry(22.0, 3.0, 26.0), brickMat);
      marketBase.position.set(174.0, 1.5, 285.0);
      marketGroup.add(marketBase);

      const marketUpperMat = new THREE.MeshLambertMaterial({
        map: this.textures.createReboco('#fef3c7', 3, 3)
      });
      const marketUpper = new THREE.Mesh(new THREE.BoxGeometry(22.0, 5.0, 26.0), marketUpperMat);
      marketUpper.position.set(174.0, 5.5, 285.0);
      marketGroup.add(marketUpper);

      // Vaulted Arched Roof
      const roofArch = new THREE.Mesh(
        new THREE.CylinderGeometry(11.0, 11.0, 26.0, 16, 1, false, 0, Math.PI),
        terracottaRoofMat
      );
      roofArch.rotation.z = Math.PI / 2;
      roofArch.rotation.y = Math.PI / 2;
      roofArch.position.set(174.0, 8.0, 285.0);
      marketGroup.add(roofArch);

      // Large Official Signboard
      const signMat = new THREE.MeshBasicMaterial({
        map: this.textures.createMercadoPyritubaSign()
      });
      const marketSign = new THREE.Mesh(new THREE.BoxGeometry(0.2, 2.5, 10.0), signMat);
      marketSign.position.set(162.8, 6.8, 285.0);
      marketGroup.add(marketSign);

      // Entrance Opening & Awnings
      const awningMat = new THREE.MeshLambertMaterial({ color: 0x16a34a });
      const awning = new THREE.Mesh(new THREE.BoxGeometry(2.5, 0.2, 8.0), awningMat);
      awning.position.set(161.5, 3.4, 285.0);
      marketGroup.add(awning);

      // Produce Display Crates (Oranges, Bananas, Melons)
      const crateWoodMat = new THREE.MeshLambertMaterial({ color: 0xb45309 });
      [-2.4, 0, 2.4].forEach((dz, idx) => {
        const crate = new THREE.Mesh(new THREE.BoxGeometry(1.2, 0.8, 1.4), crateWoodMat);
        crate.position.set(161.6, 0.4, 285.0 + dz);
        marketGroup.add(crate);

        // Colored fruit mounds
        const fruitColor = idx === 0 ? 0xf97316 : (idx === 1 ? 0xfacc15 : 0x22c55e);
        const fruitMound = new THREE.Mesh(
          new THREE.SphereGeometry(0.5, 8, 8),
          new THREE.MeshLambertMaterial({ color: fruitColor })
        );
        fruitMound.position.set(161.6, 0.9, 285.0 + dz);
        marketGroup.add(fruitMound);
      });

      this.scene.add(marketGroup);

      this.physics.addBoxCollider(
        new THREE.Vector3(162.0, 0, 268.0),
        new THREE.Vector3(188.0, 11.0, 300.0),
        'solid'
      );
    }

    // -------------------------------------------------------------------------
    // 7. EAST SIDE: RESTAURANTE AMIGOS DO PICUÍ (Z = 305 to 330, X = 160 to 188)
    // -------------------------------------------------------------------------
    {
      const restGroup = new THREE.Group();
      restGroup.name = 'amigosDoPicuiGroup';

      const restWallMat = new THREE.MeshLambertMaterial({
        map: this.textures.createReboco('#78350f', 3, 3)
      });

      // Main Restaurant Building
      const restBody = new THREE.Mesh(new THREE.BoxGeometry(22.0, 7.0, 22.0), restWallMat);
      restBody.position.set(174.0, 3.5, 318.0);
      restGroup.add(restBody);

      // Overhanging Roof
      const roofOverhang = new THREE.Mesh(new THREE.BoxGeometry(23.0, 0.8, 23.0), terracottaRoofMat);
      roofOverhang.position.set(174.0, 7.2, 318.0);
      restGroup.add(roofOverhang);

      // Large Official Signboard
      const signMat = new THREE.MeshBasicMaterial({
        map: this.textures.createAmigosDoPicuiSign()
      });
      const restSign = new THREE.Mesh(new THREE.BoxGeometry(0.2, 2.5, 10.0), signMat);
      restSign.position.set(162.8, 5.8, 318.0);
      restGroup.add(restSign);

      // Front Veranda with Wooden Tables and Checkered Cloths
      const pergolaMat = new THREE.MeshLambertMaterial({ color: 0x451a03 });
      const verandaRoof = new THREE.Mesh(new THREE.BoxGeometry(3.0, 0.2, 14.0), terracottaRoofMat);
      verandaRoof.position.set(161.5, 3.5, 318.0);
      restGroup.add(verandaRoof);

      // Timber Support Posts
      [-6.0, 0, 6.0].forEach(dz => {
        const post = new THREE.Mesh(new THREE.BoxGeometry(0.2, 3.5, 0.2), pergolaMat);
        post.position.set(160.2, 1.75, 318.0 + dz);
        restGroup.add(post);
      });

      // 3 Outdoor Tables
      const tableWoodMat = new THREE.MeshLambertMaterial({ color: 0x78350f });
      const clothMat = new THREE.MeshLambertMaterial({ color: 0xdc2626 }); // Red checkered
      [-4.5, 0, 4.5].forEach(dz => {
        const table = new THREE.Mesh(new THREE.BoxGeometry(1.2, 0.75, 1.8), tableWoodMat);
        table.position.set(161.4, 0.375, 318.0 + dz);
        restGroup.add(table);

        const cloth = new THREE.Mesh(new THREE.BoxGeometry(1.25, 0.04, 1.85), clothMat);
        cloth.position.set(161.4, 0.77, 318.0 + dz);
        restGroup.add(cloth);

        // Clay Pepper Pot (Pimenta)
        const pot = new THREE.Mesh(
          new THREE.CylinderGeometry(0.08, 0.12, 0.2, 8),
          new THREE.MeshLambertMaterial({ color: 0xc2410c })
        );
        pot.position.set(161.4, 0.88, 318.0 + dz);
        restGroup.add(pot);
      });

      this.scene.add(restGroup);

      this.physics.addBoxCollider(
        new THREE.Vector3(162.0, 0, 304.0),
        new THREE.Vector3(188.0, 9.0, 330.0),
        'solid'
      );
    }

    // -------------------------------------------------------------------------
    // 8. WEST SIDE: DROGARIA & FARMÁCIA PETRÔNIO (Z = 205 to 230, X = 115 to 131)
    // -------------------------------------------------------------------------
    {
      const farmGroup = new THREE.Group();
      farmGroup.name = 'farmaciaPetronioGroup';

      const farmWallMat = new THREE.MeshLambertMaterial({
        map: this.textures.createPaintedWall('#ffffff', 3, 3)
      });
      const farmBlueMat = new THREE.MeshLambertMaterial({ color: 0x0284c7 });

      // Main Pharmacy Storefront Body
      const farmBody = new THREE.Mesh(new THREE.BoxGeometry(16.0, 7.0, 22.0), farmWallMat);
      farmBody.position.set(123.0, 3.5, 218.0);
      farmGroup.add(farmBody);

      // Blue Accent Border
      const farmTrim = new THREE.Mesh(new THREE.BoxGeometry(16.2, 0.6, 22.2), farmBlueMat);
      farmTrim.position.set(123.0, 6.8, 218.0);
      farmGroup.add(farmTrim);

      // Glass Display Window
      const win = new THREE.Mesh(new THREE.BoxGeometry(0.1, 2.8, 12.0), windowGlassMat);
      win.position.set(131.1, 2.2, 218.0);
      farmGroup.add(win);

      // Large Official Signboard
      const signMat = new THREE.MeshBasicMaterial({
        map: this.textures.createFarmaciaPetronioSign()
      });
      const farmSign = new THREE.Mesh(new THREE.BoxGeometry(0.2, 2.2, 9.0), signMat);
      farmSign.position.set(131.2, 5.2, 218.0);
      farmGroup.add(farmSign);

      // Illuminated Green Cross on Exterior Pole
      const greenCrossMat = new THREE.MeshBasicMaterial({ color: 0x10b981 });
      const crossPost = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.06, 4.5, 8), curbMat);
      crossPost.position.set(131.5, 2.25, 210.0);
      farmGroup.add(crossPost);

      const crossV = new THREE.Mesh(new THREE.BoxGeometry(0.1, 1.2, 0.35), greenCrossMat);
      crossV.position.set(131.5, 4.2, 210.0);
      farmGroup.add(crossV);
      const crossH = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.35, 1.2), greenCrossMat);
      crossH.position.set(131.5, 4.2, 210.0);
      farmGroup.add(crossH);

      this.scene.add(farmGroup);

      this.physics.addBoxCollider(
        new THREE.Vector3(115.0, 0, 205.0),
        new THREE.Vector3(131.0, 8.0, 230.0),
        'solid'
      );
    }

    // -------------------------------------------------------------------------
    // 9. WEST SIDE: PARQUE LINEAR & PRAÇA DA COLINA (Z = 235 to 275, X = 110 to 131)
    // -------------------------------------------------------------------------
    {
      const parkGroup = new THREE.Group();
      parkGroup.name = 'pracaDaColinaGroup';

      // Park Grass Lawn
      const lawn = new THREE.Mesh(new THREE.PlaneGeometry(21.0, 40.0), grassMat);
      lawn.rotation.x = -Math.PI / 2;
      lawn.position.set(120.5, 0.26, 255.0);
      parkGroup.add(lawn);

      // Stone Walking Path
      const pathMat = new THREE.MeshLambertMaterial({ color: 0xd4a373 });
      const path = new THREE.Mesh(new THREE.PlaneGeometry(2.4, 38.0), pathMat);
      path.rotation.x = -Math.PI / 2;
      path.position.set(125.0, 0.27, 255.0);
      parkGroup.add(path);

      // Park Benches
      [242.0, 252.0, 262.0, 270.0].forEach(bz => {
        const bench = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.5, 2.0), woodMat);
        bench.position.set(127.5, 0.5, bz);
        parkGroup.add(bench);
      });

      // Trees in Park (Golden Ipê)
      this.buildIpeTree(118.0, 0.25, 242.0);
      this.buildIpeTree(115.0, 0.25, 255.0);
      this.buildIpeTree(118.0, 0.25, 268.0);

      // Playground: Swing Set (Balanço Infantil)
      const swingFrameMat = new THREE.MeshLambertMaterial({ color: 0xfacc15 });
      const swingBeam = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.15, 3.2), swingFrameMat);
      swingBeam.position.set(120.0, 2.4, 250.0);
      parkGroup.add(swingBeam);

      [-1.4, 1.4].forEach(dz => {
        const legA = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 2.6, 6), swingFrameMat);
        legA.position.set(119.5, 1.2, 250.0 + dz);
        legA.rotation.z = 0.2;
        parkGroup.add(legA);
        const legB = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 2.6, 6), swingFrameMat);
        legB.position.set(120.5, 1.2, 250.0 + dz);
        legB.rotation.z = -0.2;
        parkGroup.add(legB);
      });

      // Playground Slide (Escorregador Vermelho)
      const slideMat = new THREE.MeshLambertMaterial({ color: 0xdc2626 });
      const slideChute = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.1, 2.8), slideMat);
      slideChute.position.set(120.0, 1.0, 260.0);
      slideChute.rotation.x = -0.4;
      parkGroup.add(slideChute);

      this.scene.add(parkGroup);
    }

    // -------------------------------------------------------------------------
    // 10. WEST SIDE: SOBRADOS COMERCIAIS DO SUL (Z = 280 to 330, X = 115 to 131)
    // -------------------------------------------------------------------------
    {
      const southSobradoMat1 = new THREE.MeshLambertMaterial({
        map: this.textures.createSobradoFacade('#d97706', '#1e293b')
      });
      const sobrado1 = new THREE.Mesh(new THREE.BoxGeometry(16.0, 7.5, 24.0), southSobradoMat1);
      sobrado1.position.set(123.0, 3.75, 292.0);
      this.scene.add(sobrado1);

      const southSobradoMat2 = new THREE.MeshLambertMaterial({
        map: this.textures.createSobradoFacade('#0284c7', '#0f172a')
      });
      const sobrado2 = new THREE.Mesh(new THREE.BoxGeometry(16.0, 7.5, 24.0), southSobradoMat2);
      sobrado2.position.set(123.0, 3.75, 317.0);
      this.scene.add(sobrado2);

      // Rooftop Blue Water Tanks (Caixas d'água)
      const waterTankMat = new THREE.MeshLambertMaterial({ color: 0x1d4ed8 });
      [292.0, 317.0].forEach(tz => {
        const tank = new THREE.Mesh(new THREE.CylinderGeometry(0.8, 0.8, 1.2, 12), waterTankMat);
        tank.position.set(121.0, 8.1, tz);
        this.scene.add(tank);
      });

      this.physics.addBoxCollider(
        new THREE.Vector3(115.0, 0, 280.0),
        new THREE.Vector3(131.0, 9.0, 330.0),
        'solid'
      );
    }

    // -------------------------------------------------------------------------
    // 11. WEST & EAST PERIMETER BOUNDARY COLLIDERS (Z = 178.0 to 335.0)
    // -------------------------------------------------------------------------
    this.physics.addBoxCollider(new THREE.Vector3(98.0, 0, 178.0), new THREE.Vector3(103.0, 12.0, 335.0), 'solid');
    this.physics.addBoxCollider(new THREE.Vector3(188.0, 0, 178.0), new THREE.Vector3(195.0, 12.0, 335.0), 'solid');
  }
  ...
  // =========================================================================
  buildMapBoundaryIllusionWallsAndConstructionSites() {
    const boundaryGroup = new THREE.Group();
    boundaryGroup.name = 'boundaryIllusionWallsGroup';
    ...
    const buildHugeObrasSign = (x, y, z, rotY) => {
      ...
      signGroup.name = 'obrasHugeSign';

      // 10m x 5m panel board
      const signMat = new THREE.MeshBasicMaterial({
        map: this.textures.createObrasSignHuge()
      });
      const board = new THREE.Mesh(new THREE.BoxGeometry(10.0, 5.0, 0.2), signMat);
      board.position.set(0, 0, 0);
      signGroup.add(board);

      // Back frame
      const backFrame = new THREE.Mesh(new THREE.BoxGeometry(10.2, 5.2, 0.1), steelMat);
      backFrame.position.set(0, 0, -0.12);
      signGroup.add(backFrame);

      // Two heavy steel pillars supporting the board
      [-4.2, 4.2].forEach(px => {
        const pillar = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.22, y + 2.5, 8), steelMat);
        pillar.position.set(px, -(y + 2.5) / 2 + 2.5, 0);
        signGroup.add(pillar);
      });

      // Twin yellow hazard lamps on top
      [-3.5, 3.5].forEach(lx => {
        const lamp = new THREE.Mesh(new THREE.CylinderGeometry(0.15, 0.15, 0.35, 8), new THREE.MeshBasicMaterial({ color: 0xfacc15 }));
        lamp.position.set(lx, 2.7, 0);
        signGroup.add(lamp);
      });

      signGroup.position.set(x, y, z);
      signGroup.rotation.y = rotY;
      boundaryGroup.add(signGroup);

      this.constructionSigns.push({
        group: signGroup,
        text: 'Desculpe pelo transtorno, estamos em Obras',
    ...
    // Helper: Traffic cone
    const addCone = (cx, cy, cz) => {
      const cone = new THREE.Mesh(new THREE.ConeGeometry(0.24, 0.85, 10), coneMat);
      cone.position.set(cx, cy + 0.425, cz);
      boundaryGroup.add(cone);
      const ring = new THREE.Mesh(new THREE.CylinderGeometry(0.16, 0.20, 0.22, 10), whiteMat);
      ring.position.set(cx, cy + 0.40, cz);
      boundaryGroup.add(ring);
    };

    // Helper: Stack of concrete pipes
    const addPipes = (px, py, pz, rotY = 0) => {
      const pipeGeo = new THREE.CylinderGeometry(0.65, 0.65, 2.6, 12, 1, true);
      [-0.7, 0.7].forEach(offset => {
        const p = new THREE.Mesh(pipeGeo, pipeMat);
        p.rotation.z = Math.PI / 2;
        p.rotation.y = rotY;
        p.position.set(px + offset * Math.cos(rotY), py + 0.65, pz + offset * Math.sin(rotY));
        boundaryGroup.add(p);
      });
      const topP = new THREE.Mesh(pipeGeo, pipeMat);
      topP.rotation.z = Math.PI / 2;
      topP.rotation.y = rotY;
      topP.position.set(px, py + 1.8, pz);
      boundaryGroup.add(topP);
    };

    // Helper: Sand / gravel heap
    const addHeap = (hx, hy, hz, radius = 2.4, height = 1.3, isGravel = false) => {
      const heap = new THREE.Mesh(
        new THREE.ConeGeometry(radius, height, 10),
        isGravel ? gravelMat : sandMat
      );
      heap.position.set(hx, hy + height / 2, hz);
      boundaryGroup.add(heap);
    };

    // Helper: Scaffolding tower
    const addScaffold = (sx, sy, sz) => {
      const frame = new THREE.Group();
      [-1.2, 1.2].forEach(fx => {
        [-0.8, 0.8].forEach(fz => {
          const leg = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 4.8, 6), steelMat);
          leg.position.set(fx, 2.4, fz);
          frame.add(leg);
        });
      });
      [2.2, 4.4].forEach(dy => {
        const plank = new THREE.Mesh(new THREE.BoxGeometry(2.6, 0.08, 1.8), new THREE.MeshLambertMaterial({ color: 0xa16207 }));
        plank.position.set(0, dy, 0);
        frame.add(plank);
      });
      frame.position.set(sx, sy, sz);
      boundaryGroup.add(frame);
    };
    ...
    // -----------------------------------------------------------------------
    {
      ...
      this.physics.addBoxCollider(
        ...
        new THREE.Vector3(-59.2, 13.0, 39.0),
        'solid'
      );
      ...
      addCone(-56.8, 0, 24.0);
      addCone(-56.8, 0, 28.0);
      addPipes(-57.5, 0, 6.5, Math.PI / 2);
      addHeap(-56.2, 0, 32.5, 2.8, 1.4, false);
      addHeap(-58.0, 0, 34.0, 2.0, 1.1, true);
      addScaffold(-57.2, 0, 3.2);
    }
    ...
    // -----------------------------------------------------------------------
    {
      ...
      this.physics.addBoxCollider(
        ...
        new THREE.Vector3(166.5, 13.0, 39.0),
        'solid'
      );
      ...
      addCone(161.8, 0, 24.0);
      addCone(161.8, 0, 28.0);
      addPipes(162.5, 0, 33.5, -Math.PI / 2);
      addHeap(161.5, 0, 7.5, 2.6, 1.3, true);
      addScaffold(162.2, 0, 35.5);
    }
    ...
    // -----------------------------------------------------------------------
    {
      ...
      this.physics.addBoxCollider(
        ...
        new THREE.Vector3(156.0, 11.0, -17.5),
        'solid'
      );
      ...
      buildHugeObrasSign(145.0, 5.5, -13.5, 0);

      addCone(140.0, 0, -15.0);
      addCone(145.0, 0, -15.0);
      addCone(150.0, 0, -15.0);
      addPipes(137.0, 0, -14.5, 0);
      addHeap(152.0, 0, -14.5, 2.4, 1.2, false);
    }
    ...
    // -----------------------------------------------------------------------
    {
      ...
      this.physics.addBoxCollider(
        ...
        new THREE.Vector3(156.0, 11.0, 331.5),
        'solid'
      );
      ...
      buildHugeObrasSign(145.0, 5.5, 325.5, Math.PI);

      addCone(140.0, 0, 327.0);
    ...
    // -----------------------------------------------------------------------
    {
      ...
      this.physics.addBoxCollider(
        ...
        new THREE.Vector3(19.5, 19.5, 236.5),
        'solid'
      );
      ...
      addCone(1.5, 8.5, 231.5);
      addCone(6.0, 8.5, 231.5);
      addHeap(-12.0, 8.5, 232.0, 2.6, 1.2, true);
    }
    ...
    // -----------------------------------------------------------------------
    {
      ...
      this.physics.addBoxCollider(
        ...
        new THREE.Vector3(22.5, 18.0, -115.2),
        'solid'
      );
      ...
      buildHugeObrasSign(0.0, 9.5 + 4.8, -112.5, 0);

      addCone(-10.0, 9.5, -113.5);
      addCone(-5.0, 9.5, -113.5);
      addCone(5.0, 9.5, -113.5);
      addCone(10.0, 9.5, -113.5);
      addHeap(-16.0, 9.5, -113.5, 2.2, 1.1, false);
      addPipes(16.0, 9.5, -113.5, 0);
    }
  }
}
